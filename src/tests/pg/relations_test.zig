...
test "pg one-to-one relationships" {
    var db = try orm.postgres.PostgreSQL.init(CONN_STR);
    defer db.deinit();
    try db.exec("DROP TABLE IF EXISTS profiles_rel");
    try db.exec("DROP TABLE IF EXISTS users_rel");
    try db.exec("CREATE TABLE users_rel (id SERIAL PRIMARY KEY, name TEXT)");
    try db.exec("CREATE TABLE profiles_rel (id SERIAL PRIMARY KEY, user_id INTEGER, bio TEXT)");
    ...
    const Users = orm.Table(User, "users_rel");
    const Profiles = orm.Table(Profile, "profiles_rel");
...
test "pg one-to-many relationships" {
    var db = try orm.postgres.PostgreSQL.init(CONN_STR);
    defer db.deinit();
    try db.exec("DROP TABLE IF EXISTS posts_rel");
    try db.exec("DROP TABLE IF EXISTS users_rel");
    try db.exec("CREATE TABLE users_rel (id SERIAL PRIMARY KEY, name TEXT)");
    try db.exec("CREATE TABLE posts_rel (id SERIAL PRIMARY KEY, user_id INTEGER, title TEXT, content TEXT)");
    ...
    const Users = orm.Table(User, "users_rel");
    const Posts = orm.Table(Post, "posts_rel");
    const Repo = orm.Repo(orm.postgres.PostgreSQL);
