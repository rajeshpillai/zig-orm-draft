...
test "aggregates - count" {
    ...
    try repo.adapter.exec("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT, age INTEGER, active BOOLEAN)");
...
test "aggregates - group by" {
    ...
    // We expect 2 rows: (true, 2) and (false, 1) or similar.
    // Since we don't have a good way to map multiple columns in repo.all if it's not the model,
    // we use raw query for verification of the SQL result if needed,
    // but here we just want to verify if it compiles and runs.
